...
def login_view(request):
    # 이미 로그인되어 있다면 feeds 페이지로 이동
    if request.user.is_authenticated:
        return redirect("posts.feeds")
    
    if request.method == "POST":
        # LoginForm 인스턴스를 만들며, 입력 데이터는 request.POST 사용
        ...
        if form.is_valid():
            ...
            # 해당 사용자가 존재한다면
            if user:
                # 로그인 처리 후, feeds 페이지로 redirect
                login(request, user)

                return redirect("posts:feeds")
            
            # 사용자가 없다면, 혹은 비밀번호가 틀렸다면 form에 에러 추가
            else:
...
def follow(request, user_id):
    # 현재 로그인을 한 유저의 정보를 수집
    user = request.user
    # user id는 팔로우 대상의 id
    target_user = get_object_or_404(User, id=user_id)

    # 팔로우 하려는 유저가 이미 자신의 팔로잉 목록에 있는 경우 목록에서 제거
    if target_user in user.following.all():
        user.following.remove(target_user)

    # 팔로우 하려는 유저가 자신의 팔로잉 목록에 없는 경우 목록에 추가
    else:
        user.following.add(target_user)

    # 팔로우 토글 작업 이후 이동할 URL이 전달되었다면 해당 주소로
